...
class SocialTaskProjectMember(Base):
    __tablename__ = "social_task_project_members"
    __table_args__ = (
        UniqueConstraint("project_id", "actor_key", name="uq_social_task_project_member"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    project_id: Mapped[int] = mapped_column(Integer, ForeignKey("social_task_projects.id"), index=True)
    actor_key: Mapped[str] = mapped_column(String(60), index=True)
    added_by_key: Mapped[str] = mapped_column(String(60), default="", index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    project: Mapped["SocialTaskProject"] = relationship()
...
class SocialTask(Base):
    ...
    task_kind: Mapped[str] = mapped_column(String(20), default="company", index=True)  # company | personal
...
class SocialCalendarEvent(Base):
    ...
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), index=True)
    actor_key: Mapped[str] = mapped_column(String(60), index=True)
    entry_type: Mapped[str] = mapped_column(String(20), default="event", index=True)
