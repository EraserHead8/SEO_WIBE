...
function t(key, fallback = "") {
  ...
  return decodePossiblyMojibake(pack[key] || fallback || key);
}

function _mojibakeScore(text) {
  const value = String(text || "");
  if (!value) return 0;
  const layeredMatches = value.match(/(?:[\u0420\u0421\u0412\u0413\u0432\u0440][\u0080-\u04FF]){2,}/g);
  ...
  const questionScore = (value.match(/\?{2,}/g) || [])
    .reduce((acc, item) => acc + String(item || "").length, 0) * 2;
  return layeredScore + markerScore + replacementScore + questionScore;
}
...
function _collectMojibakeCandidates(raw) {
  const start = String(raw ?? "");
  if (!start) return [""];
  const seen = new Set([start]);
  const queue = [start];
  const out = [start];
  const decoders = [_cp1251DecodeUtf8, _latin1DecodeUtf8];
  for (let depth = 0; depth < 3 && queue.length; depth += 1) {
    const level = queue.splice(0, queue.length);
    for (const current of level) {
      for (const decode of decoders) {
        const next = String(decode(current) || "");
        if (!next || next === current || seen.has(next)) continue;
        seen.add(next);
        out.push(next);
        queue.push(next);
      }
...
function decodePossiblyMojibake(input) {
  ...
  const candidates = _collectMojibakeCandidates(raw);
  let best = raw;
...
function normalizePossiblyMojibakeDeep(input, depth = 0, seen = null) {
  if (depth > 6 || input == null) return input;
  if (typeof input === "string") return decodePossiblyMojibake(input);
  if (Array.isArray(input)) return input.map((item) => normalizePossiblyMojibakeDeep(item, depth + 1, seen));
  if (typeof input !== "object") return input;
  const nextSeen = seen || new WeakSet();
  if (nextSeen.has(input)) return input;
  nextSeen.add(input);
  const out = {};
  for (const [key, value] of Object.entries(input)) {
    out[key] = normalizePossiblyMojibakeDeep(value, depth + 1, nextSeen);
  }
  return out;
}

function normalizePossiblyMojibakeDom(root = null) {
  if (typeof document === "undefined") return;
  const base = root && root.nodeType ? root : document.body;
  if (!base) return;

  const applyAttrs = (el) => {
    if (!el || el.nodeType !== 1) return;
    ["title", "placeholder", "aria-label", "alt", "data-tip"].forEach((name) => {
      if (!el.hasAttribute(name)) return;
      const current = String(el.getAttribute(name) || "");
      const fixed = decodePossiblyMojibake(current);
      if (fixed && fixed !== current) el.setAttribute(name, fixed);
    });
    ...
    if (tag === "INPUT") {
      ...
      if (!["button", "submit", "reset", "checkbox", "radio", "range", "color", "date", "datetime-local", "time", "week", "month", "number", "file", "hidden", "password"].includes(type)) {
        const current = String(el.value || "");
        const fixed = decodePossiblyMojibake(current);
        if (fixed && fixed !== current) el.value = fixed;
      }
    }
  };

  if (base.nodeType === 3) {
    const current = String(base.nodeValue || "");
    const fixed = decodePossiblyMojibake(current);
    if (fixed && fixed !== current) base.nodeValue = fixed;
    return;
  }
  ...
  if (document.createTreeWalker) {
    ...
    while (walker.nextNode()) {
      ...
      if (!textNode || ["SCRIPT", "STYLE", "NOSCRIPT", "TEXTAREA"].includes(parentTag)) continue;
      const current = String(textNode.nodeValue || "");
      if (!current.trim()) continue;
      const fixed = decodePossiblyMojibake(current);
      if (fixed && fixed !== current) textNode.nodeValue = fixed;
    }
  }
  base.querySelectorAll?.("input,textarea,[title],[placeholder],[aria-label],[alt],[data-tip]").forEach(applyAttrs);
}

let _mojibakeDomObserver = null;
let _mojibakeDomQueued = false;

function queueMojibakeDomNormalize(root = null) {
  if (typeof document === "undefined") return;
  const target = root && root.nodeType ? root : document.body;
  if (!target || _mojibakeDomQueued) return;
  _mojibakeDomQueued = true;
  const run = () => {
    _mojibakeDomQueued = false;
    normalizePossiblyMojibakeDom(target);
  };
...
function installMojibakeDomObserver() {
  if (typeof MutationObserver === "undefined" || typeof document === "undefined" || _mojibakeDomObserver) return;
  const root = document.body;
  if (!root) return;
  _mojibakeDomObserver = new MutationObserver((mutations) => {
    for (const mutation of mutations || []) {
      if (mutation.type === "characterData" && mutation.target) {
        queueMojibakeDomNormalize(mutation.target);
        continue;
      }
      ...
      for (const node of mutation.addedNodes || []) {
        queueMojibakeDomNormalize(node);
      }
    }
  });
  _mojibakeDomObserver.observe(root, {
    childList: true,
    subtree: true,
    characterData: true,
    attributes: true,
...
if (typeof window !== "undefined") {
  ...
  window.normalizePossiblyMojibakeDeep = normalizePossiblyMojibakeDeep;
  window.normalizePossiblyMojibakeDom = normalizePossiblyMojibakeDom;
  window.queueMojibakeDomNormalize = queueMojibakeDomNormalize;
}
...
function applyUiLanguage() {
  ...
  const setText = (selector, value) => {
    ...
    if (el) el.textContent = decodePossiblyMojibake(value);
  };
  const setTextAll = (selector, value) => {
    document.querySelectorAll(selector).forEach((el) => {
      el.textContent = decodePossiblyMojibake(value);
    });
  };
  const setOptions = (selector, labels) => {
    ...
    labels.forEach((label, idx) => {
      if (el.options[idx]) el.options[idx].textContent = decodePossiblyMojibake(label);
    });
  };
  const setCheckLabel = (selector, labelText) => {
    ...
    el.append(document.createTextNode(` ${decodePossiblyMojibake(labelText)}`));
  };
  ...
  for (const [selector, text] of placeholders) {
    const el = document.querySelector(selector);
    if (el) el.placeholder = decodePossiblyMojibake(text);
  }
...
async function requestJson(url, opts = {}) {
  ...
  const makeRequestError = (message, extra = {}) => {
    const err = new Error(
      decodePossiblyMojibake(String(message || (currentLang === "en" ? "Request error" : "\u041e\u0448\u0438\u0431\u043a\u0430 \u0437\u0430\u043f\u0440\u043e\u0441\u0430")))
    );
    ...
    if (typeof extra.rawText === "string") {
      err.rawText = decodePossiblyMojibake(extra.rawText);
      err.raw_text = err.rawText;
    }
  ...
  const parseResponseBody = async (response) => {
    ...
    const rawText = compactRawText(decodePossiblyMojibake(text) || text);
    if (!String(text || "").trim()) {
      return {
        ok: true,
        hasBody: false,
    ...
    try {
      return {
        ok: true,
        hasBody: true,
        payload: normalizePossiblyMojibakeDeep(JSON.parse(text)),
        rawText,
      };
    } catch (parseError) {
      return {
