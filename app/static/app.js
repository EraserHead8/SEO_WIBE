...
async function requestJson(url, opts = {}) {
  ...
  throw lastError || new Error(currentLang === "en" ? "Request error" : "Р С›РЎв‚¬Р С‘Р В±Р С”Р В° Р В·Р В°Р С—РЎР‚Р С•РЎРѓР В°");
}
...
function buildLoadStatusHtml({ title = "", loaded = 0, total = 0, active = false, failed = 0 }) {
  ...
  const failedText = failed > 0 ? ` • ${tr("РѕС€РёР±РѕРє", "errors")}: ${failed}` : "";
  return `
...
function isPlaceholderCampaignName(name, campaignId = "") {
  ...
  if (!text) return true;
  const low = text.toLowerCase();
  if (/^(РєР°РјРїР°РЅРёСЏ|campaign)\s*\d+$/.test(low)) return true;
  const cid = String(campaignId || "").trim().toLowerCase();
  if (cid && (low === `РєР°РјРїР°РЅРёСЏ ${cid}` || low === `campaign ${cid}`)) return true;
  return false;
}
...
function syncWbBidderTargetKindUi() {
  ...
  if (kind === "nm") {
    ...
    targetValueEl.placeholder = tr("Р”Р»СЏ nm РЅРµ РёСЃРїРѕР»СЊР·СѓРµС‚СЃСЏ", "Not used for nm");
    targetValueEl.value = "";
    return;
  }
  targetValueEl.disabled = false;
  targetValueEl.placeholder = tr("Р¤СЂР°Р·Р° (РґР»СЏ normquery)", "Phrase (for normquery)");
}
...
async function openProductViewModal(productId, opts = {}) {
  ...
  if (metaEl) {
    metaEl.textContent = product
      ? `${tr("ID", "ID")}: ${product.id} • ${tr("РњРџ", "MP")}: ${String(product.marketplace || "").toUpperCase()} • ${tr("РљР°С‚РµРіРѕСЂРёСЏ", "Category")}: ${product.category_name || "-"}`
      : "-";
  }
...
async function loadSalesStats(retryAttempt = 0, forceRefresh = false) {
  ...
  if (!data) {
    ...
    updateSalesLoadStatus();
    renderSalesStats();
    if (meta) meta.textContent = tr("РћС€РёР±РєР° Р·Р°РіСЂСѓР·РєРё СЃС‚Р°С‚РёСЃС‚РёРєРё. РџСЂРѕРІРµСЂСЊС‚Рµ API-РєР»СЋС‡Рё Рё РїРµСЂРёРѕРґ.", "Sales loading failed. Check API keys and period.");
