...
function renderAccountingMonthlySummary() {
  ...
  try {
    ...
    if (warnings.length) {
      ...
      if (warnings.length > 2) statusParts.push(tr(`(+ ${warnings.length - 2} ещё предупреждений)`, `(+ ${warnings.length - 2} more warnings)`));
    }
...
function applyAccountingUiLanguageFixups() {
  if (currentLang === "en") return;
  const setText = (selector, value) => {
    const el = document.querySelector(selector);
    if (el) el.textContent = value;
  };
  setText("#accountingSubtabOverviewBtn", "Обзор");
  setText("#accountingSubtabAnalysisBtn", "Анализ");
  setText("#accountingSubtabMonthlyBtn", "Прибыль по месяцам");
  setText("#accountingSubtabExpensesBtn", "Расходы");
  setText("#accountingSubtabSettingsBtn", "Налоги и параметры");
  setText("#accountingSubtabOverview .panel h3", "Прибыль и обзор");
  setText("#accountingSubtabMonthly .panel h3", "Прибыль по месяцам по маркетплейсам");
  setText("#accountingSubtabOverview .grid-6 button:nth-of-type(1)", "Обновить данные");
  setText("#accountingSubtabOverview .grid-6 button:nth-of-type(2)", "Обновить все");
  setText("#accountingSubtabOverview .accounting-file-actions button:nth-of-type(1)", "Скачать шаблон Excel");
  setText("#accountingSubtabOverview .accounting-file-actions button:nth-of-type(2)", "Экспорт цен");
  setText("#accountingSubtabOverview .accounting-file-actions button:nth-of-type(3)", "Импорт закупочных цен");
  setText("#accountingSubtabAnalysis .panel h3", "Товарная аналитика");
  setText("#accountingSubtabAnalysis .grid-4 button:nth-of-type(1)", "Обновить анализ");
  setText("#accountingSubtabAnalysis .grid-4 button:nth-of-type(2)", "Сбросить фильтры");
  setText("#accountingSubtabExpenses .panel h3", "Учет расходов");
  setText("#accountingSubtabExpenses .actions button:nth-of-type(1)", "Сохранить расход");
  setText("#accountingSubtabExpenses .actions button:nth-of-type(2)", "Сбросить форму");
  setText("#accountingSubtabSettings .panel h3", "Налоги и параметры расчета");
  setText("#accountingSubtabSettings .accounting-settings-grid .field-label:nth-of-type(1) > span", "Ставка НДС, %");
  setText("#accountingSubtabSettings .accounting-settings-grid .field-label:nth-of-type(2) > span", "Налог на прибыль/УСН, %");
  setText("#accountingSubtabSettings .accounting-settings-grid .field-label:nth-of-type(3) > span", "Доп. расходы от выручки, %");
  setText("#accountingSubtabSettings .accounting-settings-grid .field-label:nth-of-type(4) > span", "Фикс. расходы в месяц, ₽");
  setText("#accountingSubtabSettings .panel > .hint", "НДС и налог применяются к итоговой прибыли по выбранному периоду. Если параметр не используется, оставьте 0.");
  setText("#accountingSubtabSettings .actions button:nth-of-type(1)", "Сохранить параметры");
  setText("#accountingSubtabSettings .actions button:nth-of-type(2)", "Обновить");

  const templateMarketSel = document.getElementById("accountingTemplateMarketplace");
  if (templateMarketSel) {
    const prev = String(templateMarketSel.value || localStorage.getItem("accounting_template_marketplace") || "all").toLowerCase();
    templateMarketSel.innerHTML = `
      <option value="all">Шаблон: WB + Ozon</option>
      <option value="wb">Шаблон: только WB</option>
      <option value="ozon">Шаблон: только Ozon</option>
    `;
    templateMarketSel.value = ["all", "wb", "ozon"].includes(prev) ? prev : "all";
  }
...
const originalApplyAccountingUiLanguage = window.applyAccountingUiLanguage;
window.applyAccountingUiLanguage = function applyAccountingUiLanguagePatched() {
  if (typeof originalApplyAccountingUiLanguage === "function") {
    originalApplyAccountingUiLanguage();
  }
  applyAccountingUiLanguageFixups();
};
