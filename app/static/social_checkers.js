(function () {
    ...
    return `
      <div class="social-checkers-room-card">
        ...
        <div class="actions">
          ...
          ${room.can_delete ? `<button class="btn-danger" type="button" onclick="socialCheckersDeleteRoom(${Number(room.id || 0)})">${checkersEsc(checkersTr("Удалить", "Delete"))}</button>` : ""}
        </div>
      </div>
    `;
  }
        ...
        <aside class="social-checkers-sidebar">
          ...
          <section class="social-checkers-panel">
            ...
            <div class="actions social-checkers-actions-stack">
              ...
              ${room.can_delete ? `<button class="btn-danger" type="button" onclick="socialCheckersDeleteRoom(${Number(room.id || 0)})">${checkersEsc(checkersTr("Удалить комнату", "Delete room"))}</button>` : ""}
  ...
  function checkersFindRoomById(roomId) {
    const id = Number(roomId || 0);
    if (!id) return null;
    const current = socialCheckersState.currentRoom;
    if (Number(current?.id || 0) === id) return current;
    const rooms = socialCheckersState.overview?.rooms || {};
    const lists = [rooms.mine, rooms.public, rooms.finished, rooms.history];
  ...
  async function socialCheckersDeleteRoom(roomId = 0) {
    const id = Number(roomId || socialCheckersState.currentRoom?.id || 0);
    if (!id) return;
    const room = checkersFindRoomById(id) || socialCheckersState.currentRoom || {};
    const title = String(room?.title || room?.room_code || "").trim();
    const question = title
      ? checkersTr(`Удалить комнату "${title}"?`, `Delete room "${title}"?`)
      : checkersTr("Удалить эту комнату?", "Delete this room?");
    if (!window.confirm(question)) return;
    try {
      await socialRequest(`/api/social/games/checkers/rooms/${id}`, {
        method: "DELETE",
        body: JSON.stringify({}),
      });
      socialCheckersState.currentRoom = null;
      socialCheckersState.selectedKey = "";
      await socialCheckersOpenMenu();
    } catch (error) {
      alert(error?.message || checkersTr("Не удалось удалить комнату", "Failed to delete room"));
    }
  }
  ...
  window.socialCheckersDeleteRoom = socialCheckersDeleteRoom;
})();
