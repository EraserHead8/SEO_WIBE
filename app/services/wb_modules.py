...
def _safe_uncertain_catalog_reply(client_text: str) -> str:
    ...
    if _is_chimney_reply_context(client_text) and _has_close_chimney_nominal_pair(client_text):
        return (
            "Здравствуйте! По номинальному размеру такой зонт может подойти, но точную посадку мы гарантируем только "
            "при стыковке элементов одной системы/производителя. Если труба другого производителя или асбестовая, "
            "нужно примерить и сверить посадку; иногда требуется небольшая доработка трубы или зонта. У дымоходных "
            "элементов одна сторона чуть больше, другая чуть меньше — это сделано для стыковки, также важно выбрать "
            "нужную сторону соединения: по дыму или по конденсату."
        )
    return (
    ...
    *,
    ...
    if safe_kind == "question" and _is_chimney_reply_context(product_name, client_text):
        ...
        rules.append(
            ...
            "Стороны часто называют мама/папа: одна сторона чуть больше, другая чуть меньше, чтобы элементы стыковались."
        )
        rules.append(
            "- Точную совместимость и посадку гарантируй только для элементов одной системы/одного производителя, например Corax с Corax. "
            "Если клиент спрашивает про чужую, асбестовую или нестандартную трубу, не обещай точную совместимость: говори, что по номиналу может подойти, "
            "но нужно примерить/сверить посадочный диаметр, сторону Д/К и иногда может потребоваться небольшая доработка трубы или зонта."
        )
