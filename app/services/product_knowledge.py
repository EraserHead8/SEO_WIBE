...
def _format_product_context(
    ...
    *,
    ...
    if missing_number_match and requested_numbers:
        ...
        parts.append(
            f"[CATALOG_SIZE_UNCONFIRMED] В обращении есть число/размер {numbers}, но у найденных карточек нет полного совпадения по всем этим размерам. "
            "Это служебная пометка: не пиши клиенту про найденные данные, контекст, точное совпадение или внутреннюю проверку. "
            "Ответь естественно: чтобы не ошибиться с подбором, нужно сверить посадочный диаметр, размер юбки/зонта и способ монтажа. "
