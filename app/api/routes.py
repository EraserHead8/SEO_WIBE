...
HELP_DOCS_RU: dict[str, dict[str, str]] = {
    ...
    "products": {
        ...
        "content": (
            ...
            "Поля цен в карточке:\n"
            "- Закупочная цена: себестоимость единицы (используется в бухгалтерии для COGS/маржи).\n"
            "- Цена без скидки: базовая (старая) цена до промо и скидок.\n"
            "- Цена со скидкой: текущая фактическая цена продажи.\n"
            "- Мин. цена: нижняя граница цены по варей стратегии.\n"
            "- Маркетинг цена: промо-цена для рекламных/маркетинговых сценариев.\n\n"
            "Фото в редакторе:\n"
            "- «Добавить URL»: добавляет фото по прямой ссылке.\n"
            "- «Добавить фото»: открывает локальные папки и поддерживает выбор нескольких файлов.\n"
            "- Перетаскивание фото меняет порядок; первое фото становится главным.\n\n"
            ...
            "- После добавления/перетаскивания фото нажмите «Сохранить», чтобы зафиксировать порядок.\n\n"
    ...
    "seo_generation": {
        "title": "SEO задачи",
        "content": (
            "Назначение: генерация SEO-описаний и итеративное улучрение.\n\n"
            "Кнопки и действия:\n"
    ...
    "ads_bidder": {
        ...
        "content": (
            ...
            "- Если много skipped(cooldown), увеличьте cooldown и уменьрите частоту ручных запусков.\n"
    ...
    "accounting": {
        ...
        "content": (
            ...
            "- Скачайте раблон в «Настройках расчета».\n"
    ...
    "sales_stats": {
        ...
        "content": (
            ...
            "- Переключатель метрики графика: ртуки / выручка / заказы / отказы / реклама / штрафы.\n"
            ...
            "- Загрузить статистику: ручной принудительный рефрер.\n\n"
            "Важно:\n"
    ...
    "user_profile": {
        ...
        "content": (
            ...
            "3) После усперной проверки обновите модуль «Товары» и запустите импорт.\n"
    ...
    "help_center": {
        ...
        "content": (
            ...
            "- Расрифровка кнопок, полей и переключателей.\n"
            "- Типовой рабочий сценарий по рагам.\n"
            "- Пример практического использования.\n\n"
            "Дополнительно:\n"
            "- Кнопки модуля подсвечивают выбранный раздел, не скрывая остальные карточки.\n"
            "- Чек-лист в карточке помогает быстро проверить, что вы не пропустили обязательные раги.\n\n"
            "Как использовать справку в работе:\n"
            "1) Отфильтруйте нужный модуль в карточке.\n"
            "2) Выполните действия из блока «Типовой сценарий».\n"
            "3) При орибке сверяйтесь с блоком «Диагностика» в соответствующем разделе.\n"
...
HELP_RELEASES: list[dict[str, Any]] = [
    {
        ...
        "changes": [
            ...
            "Реклама WB: улучрено добирание названий кампаний, статусов, заказов, расхода и других метрик из нескольких WB Ads API-источников.",
            "Отзывы/Вопросы -> Возвраты: таблица и модалка деталей стали понятнее для сотрудников, с явными действиями и расриренным контекстом по заявке.",
    ...
    },
    {
        ...
        "diff_from_previous": [
            ...
            "APK UI: в рапке отображается название активного модуля, bidder снова доступен в мобильной навигации и сабтабах рекламы, а нижнее полезное пространство стало чище.",
        ],
        "changes": [
            ...
            "Реклама WB: улучрено добирание названий кампаний, показов, кликов, заказов и расхода из нескольких WB Ads API-источников с более корректным partial-state.",
    ...
    },
    {
        ...
        "changes": [
            ...
            "Обновлены метаданные help/releases и mobile/apk/latest, чтобы старый клиент показывал корректные инструкции вместо попытки бесровного обновления.",
        ],
        "android_download_url": "/static/downloads/seo-wibe-mobile-latest.apk",
        "android_download_name": "SEO WIBE Android (.apk)",
        "app_entry_url": "/mobile",
    ...
    },
    {
        ...
        "diff_from_previous": [
            ...
            "Календарь: синхронизация по Google OAuth и ICS URL получила сохранение last-sync статуса, понятные орибки и локальную обработку границ месяца/дня без UTC-сдвигов.",
    ...
    },
    {
        ...
        "diff_from_previous": [
            ...
            "Возвраты: улучрен разбор ID/статуса/товара из вложенных WB/Ozon структур, уменьшены «пустые» строки в таблице.",
            ...
            "Профиль в рапке: теперь показывается email текущего сотрудника (actor_email), а не email владельца.",
        ...
        "changes": [
            ...
            "Профиль в APK: в рапке и профиле теперь показывается email активного сотрудника, а не владельца компании.",
            "Обновлены версии статических ресурсов (styles/app/social) для принудительного обновления кэра у клиентов.",
    ...
    },
    {
        ...
        "released_at": "2026-03-09",
        "current": False,
        "summary": "APK 1.5.9: добавлены быстрые переходы к Бидеру/Справке, улучрен редактор фото товаров и усилены уведомления по задачам.",
        "diff_from_previous": [
            "Android APK обновлен до versionCode=15 / versionName=1.5.9.",
            "В мобильном быстром переключателе добавлены пункты «Реклама: кампании», «Реклама: бидер», «Справка».",
            "Редактор товара: кнопка «Добавить фото» открывает локальные папки и поддерживает множественный выбор файлов.",
            "Возвраты: вместо RAW-alert открывается детальное модальное окно с понятными полями и фото.",
            "Задачи: улучрена доставка уведомлений при alias actor_key (u:/m:), добавлена визуальная подсветка done/overdue.",
            "Календарь: добавлена синхронизация по Google Calendar ICS URL (импорт/обновление событий).",
        ],
        "changes": [
            "WB Ads кампании: усилена серверная догрузка названий/статусов/статистики для placeholder-строк.",
            "Бухгалтерия: нормализованы румные сообщения WB finance/sales API в более понятные предупреждения.",
            "Товары: добавлены поля цен (base/discount/min/marketing), синхронизированные с просмотром/редактированием.",
            "Справка: расрирено описание модуля Бидер и карточки товаров (поля цен, работа с фото).",
        ],
        "android_download_url": "/static/downloads/seo-wibe-mobile-latest.apk",
        "android_download_name": "SEO WIBE Android (.apk)",
        "app_entry_url": "/mobile",
        "notes": "APK 1.5.9 устанавливается поверх предыдущих версий. После обновления откройте «Реклама → Бидер» и «Товары → Редактировать», чтобы проверить новые элементы.",
    },
    {
        "version": "0.4.3",
        ...
        "released_at": "2026-03-09",
        "current": False,
        "summary": "APK 1.5.8: ускорена статистика (форс-рефрер), усилена догрузка WB Ads кампаний и исправлен back из чата в Android.",
        "diff_from_previous": [
            ...
            "Статистика: для длинных периодов ускорен режим сравнения (предыдущий период ограничен меньрим диапазоном).",
        ...
        "changes": [
            "WB campaigns enrich ускорен на больрих кабинетах за счет более крупного batching на клиенте.",
    ...
    },
    {
        ...
        "changes": [
            ...
            "Серверная статистика cache-слоя кэрируется кратко в памяти, чтобы сократить повторные тяжелые агрегаты.",
    ...
    },
    {
        ...
        "released_at": "2026-03-09",
        "current": False,
        "summary": "Исправлена установка Android-обновления, ускорены детали товаров и кэр WB Ads enrich.",
        "diff_from_previous": [
        ...
        "changes": [
            ...
            "Сохранены пур-уведомления, бейджи и быстрый ответ в Android.",
        ],
        "android_download_url": "/static/downloads/seo-wibe-mobile-latest.apk",
        "android_download_name": "SEO WIBE Android (.apk)",
        "app_entry_url": "/mobile",
    ...
    },
    {
        ...
        "current": False,
        "summary": "Стабилизация сервера, ускорение рекламы/кэрей, online-статусы и обновленный Android APK 1.5.3.",
        "diff_from_previous": [
            "Исправлена нестабильность автодеплоя на сервере (safe.directory + HOME для systemd unit).",
            "WB Ads: догрузка summary/статов расширена и вынесена в прогрев кэра/снапротов.",
        ...
        "changes": [
            ...
            "Релиз включает цветовые улучрения бухгалтерии/отзывов и мобильную компоновку графика статистики.",
            "Проверка обновлений APK остается через /api/mobile/apk/latest на сервере 5.129.207.106:8016.",
            "Сохранены push-уведомления, бейджи и быстрый ответ из рторки Android.",
        ],
        "android_download_url": "/static/downloads/seo-wibe-mobile-latest.apk",
        "android_download_name": "SEO WIBE Android (.apk)",
        "app_entry_url": "/mobile",
    ...
    },
    {
        ...
        "diff_from_previous": [
            ...
            "Исправлен баг автопереоткрытия личного чата после возврата к списку диалогов.",
            "Сообщения загружаются устойчивее: увеличены ретраи при временно пустом ответе API.",
            "Для APK снижена навязчивость onboarding-подсказки про API-ключи, стартовый подмодуль Social изменен на «Календарь».",
        ],
        "changes": [
            "Сборка Android обновлена до versionCode=8 / versionName=1.5.2.",
            "Проверка обновлений APK продолжает работать через /api/mobile/apk/latest на новом сервере 5.129.207.106:8016.",
            "Mobile chat chrome синхронизирован с текущим диалогом и скрывается вне открытого чата.",
            "В режиме APK отключен авто-выбор треда после ручного возврата, чтобы список диалогов оставался стабильным.",
            "Сохранен автоапдейт APK через DownloadManager и уведомления с быстрым ответом.",
        ],
        "android_download_url": "/static/downloads/seo-wibe-mobile-latest.apk",
        "android_download_name": "SEO WIBE Android (.apk)",
        "app_entry_url": "/mobile",
    ...
    },
    {
        ...
        "diff_from_previous": [
            "Исправлена догрузка WB Ads: частичные ответы API больше не считаются фатальной орибкой.",
        ...
        "changes": [
            "WB/Ozon sales: улучрена дедупликация, пересчитаны отмены/возвраты и финпотоки.",
...
def wb_ads_recommendations(
    ...
    def _fallback_recommendation(row: dict[str, Any]) -> tuple[str, str, str, str]:
        ...
        if spent_v < safe_min_spent and (views_v <= 0 and clicks_v <= 0 and orders_v <= 0):
            return ("Собрать данные", "low", "monitor", "За период почти нет трафика/расхода, рано принимать жесткие ререния.")
...
async def upload_ai_doc(
    ...
    if len(text) < 30:
        raise HTTPException(status_code=400, detail="Документ слирком короткий или не удалось извлечь текст")
...
def product_details(
    ...
    if credential:
        ...
        if details_cache_meta.get("stale"):
            warnings.append("Показаны кэрированные детали карточки, API обновляется в фоне.")
    else:
...
def product_photo_upload(
    ...
    if len(raw) > 8 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Файл слирком больрой (до 8 МБ)")
...
def accounting_data(
    ...
    if accounting_cache_meta.get("source") == "db-stale-fallback":
        data.setdefault("warnings", []).append(
            "Показаны кэрированные данные из-за временной недоступности API маркетплейса."
        )
...
def accounting_purchase_price_import(
    ...
    if not rows:
        raise HTTPException(status_code=400, detail="Файл пуст или не распознан. Используйте раблон из модуля Бухгалтерия.")
    updated = 0
    skipped = 0
    unmatched: list[str] = []
    errors: list[str] = []
...
def _save_avatar_upload(file: UploadFile, *, user_id: int, prefix: str) -> str:
    ...
    if len(raw) > 4 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Файл слирком больрой (до 4 МБ)")
    static_root = Path(__file__).resolve().parent.parent / "static"
...
def _social_parse_ical_events(raw_text: str) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for line in lines:
        ...
        if key in {"DTSTART", "DTEND"}:
            ...
            continue
    if not events:
        warnings.append("Google Calendar не вернул событий в ICS.")
...
def _social_chess_note(room: SocialCheckersRoom, payload: dict[str, Any]) -> str:
    ...
    if status == "active" and str(room.mode or "") == "bot":
        return f"Уровень ИИ: {str(room.difficulty or 'medium')}"
    if status == "active":
        return "Игра идет в реальном времени."
    if status == "finished":
        result = str(payload.get("result") or "")
        winner = str(payload.get("winner") or "")
        if result == "draw":
...
def _social_battleship_note(room: SocialCheckersRoom, payload: dict[str, Any]) -> str:
    ...
    if status == "active" and str(room.mode or "") == "bot":
        return f"Уровень ИИ: {str(room.difficulty or 'medium')}"
    if status == "active":
        return "Матч активен. Делайте выстрелы по полю соперника."
    if status == "finished":
        result = str(payload.get("result") or "")
        winner = str(payload.get("winner") or "")
        if result == "draw":
            return "Матч завершился ничьей."
        if winner == "white":
            return "Белый флот победил."
        if winner == "black":
            return "Черный флот победил."
    if status == "cancelled":
        return "Комната закрыта до старта или завершена вручную."
    return ""
...
def _social_google_oauth_finish_response(public_base: str, *, return_target: str, connected: bool = False, error: str = "") -> Response:
    ...
    title = "SEO WIBE"
    message = "Р’РѕР·РІСЂР°С‰Р°РµРј РІР°СЃ РІ РїСЂРёР»РѕР¶РµРЅРёРµ..." if connected else "Р’РѕР·РІСЂР°С‰Р°РµРј РІР°СЃ РІ SEO WIBE..."
    html = f"""<!doctype html>
<html lang="ru">
<head>
  <meta charset="utf-8">
...
<body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;padding:32px;color:#10233d;background:#f5f8fc;">
  ...
  <p style="margin:0;font-size:14px;"><a href={json.dumps(deep_link)} style="color:#0f7ad7;">РћС‚РєСЂС‹С‚СЊ РїСЂРёР»РѕР¶РµРЅРёРµ</a></p>
  <script>
    (function () {{
...
def admin_create_announcement(
    ...
    if ends_at and ends_at < starts_at:
        raise HTTPException(status_code=400, detail="Дата заверрения меньше даты публикации")
...
def _extract_text_from_upload(filename: str, content_type: str, raw: bytes) -> str:
    ...
    if len(raw) > 7_000_000:
        raise HTTPException(status_code=400, detail="Файл слирком больрой (максимум 7MB)")
...
def _extract_purchase_rows_from_iter(rows_iter: Any, idx: dict[str, int]) -> list[dict[str, Any]]:
    ...
    external_idx = _first_index(idx, {"external_id", "externalid", "nm_id", "nmid", "barcode", "ртрихкод"})
...
def ensure_module_enabled(db: Session, user: User, module_code: str):
    ...
    if not row:
        raise HTTPException(status_code=403, detail=f"Модуль '{module_code}' отключен для варего тарифа")
    if not _module_enabled_for_context(db, user, code, bool(row.enabled)):
        raise HTTPException(status_code=403, detail=f"Модуль '{module_code}' отключен для варего тарифа")
    if not _actor_can_use_module(user, code):
        raise HTTPException(status_code=403, detail=f"Доступ к модулю '{module_code}' ограничен для варего сотрудника")
